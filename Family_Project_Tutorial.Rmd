...
As you can see, these two lines serve as potential cutoff points for the samples to the left of each line. It is a good idea to visualize the samples with a rarefaction curve like this and make a decision about where to cut off samples. After taking a closer look, a more optimal position to cut off samples would be at a sample size of ~2000. Here you can see that only the 3 samples with insufficient sample depth are not included in the final rarefied data. Here's what rarefying to ~2000 samples would look like.
```{r}
transOTU <- rowSums(t(OTU.tableFP)) 
Q03 <- quantile(transOTU[order(transOTU, decreasing = TRUE)], 0.03) #~2000 samples
#^ is this quantile notation in the beginning correct
rarecurve(t(OTU.tableFP), step = 100, cex = 0.5)
abline(v = c(Q03), col = c("purple"))
```
...
FPrared.OTU <- as.data.frame((rrarefy.perm(t(OTU.tableFP), sample = Q03, n = 100, round.out = T))) #with all samples - rarefies the data

FPrared.OTU <- as.data.frame(FPrared.OTU[rowSums(FPrared.OTU) >= Q03 - (Q03 * 0.1), colSums(FPrared.OTU) >= 1]) #with some samples cut off - removes low depth samples that do not meet the cutoff
```
## Alpha Diversity
